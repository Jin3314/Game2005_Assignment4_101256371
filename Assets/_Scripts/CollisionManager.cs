...
public class CollisionManager : MonoBehaviour
{
    ...
    public BulletBehaviour[] Bulletcube;
    ...
    void Update()
    {
        Bulletcube = FindObjectsOfType<BulletBehaviour>();
        ...
        // Check each bulletcube against each AABB in the scene
        foreach (var Bulletc in Bulletcube)
        {
            foreach (var cube in cubes)
            {
                if (cube.name != "Player")
                {
                    BulletAABB(Bulletc, cube);
                }
    ...
    public static void BulletAABB(BulletBehaviour s, CubeBehaviour b) //Bullet AABB
    {
        ...
        var z = Mathf.Max(b.min.z, Mathf.Min(s.transform.position.z, b.max.z));
         ...
         if((x <= s.transform.position.x && x >= s.transform.position.x) &&
